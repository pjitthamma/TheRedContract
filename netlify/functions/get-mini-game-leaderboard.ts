...
const json = (statusCode: number, body: unknown, cacheControl = "public, max-age=15, s-maxage=15") => ({
  statusCode,
  headers: {
...
export const handler = async (event: { queryStringParameters?: { roomKey?: string } | null }) => {
  ...
  if (!roomKey || !roomKeys.has(roomKey)) {
    return json(400, { error: "Valid roomKey is required" }, "no-store");
  }
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return json(500, { error: "Supabase environment variables are missing" }, "no-store");
  }
  ...
  if (!response.ok) {
    return json(500, { error: "Could not fetch leaderboard" }, "no-store");
  }
